...
class TestCompound2 extends StatefulWidget {
  const TestCompound2({
    Key? key,
...
class _TestCompoundPageState extends State<TestCompound2> {
  ...
  late THREE.BufferGeometry capsuleGeometry;
  ...
  Future<void> initPage() async {
    ...
    matBox = MeshPhongMaterial({'name':'box','color': THREE.Color(255,0,0),'shininess': 10});
    matSphereSleep = MeshPhongMaterial({'name':'ssph','specular': 0xFFFFFF, 'shininess': 120 , 'transparent': true, 'opacity': 0.7});
    matBoxSleep = MeshPhongMaterial( {'name':'sbox','color': THREE.Color(255,0,0),'shininess': 10});
  ...
  void initChairGeometry(double radius,double height,[double? sRadius,double? sHeight]) {
    types = [OIMO.Shapes.sphere,OIMO.Shapes.sphere,OIMO.Shapes.sphere];
    sizes = [ radius,radius,radius, radius,radius,radius, radius,radius,radius ];
    positions = [0,0,0,   0,height*0.5,0, 0,height,0];

    double sRad = sRadius ?? 20;
    double sHei = sHeight ?? 10;
    double o0 = Math.PI*2;
    double o1 = Math.PI/2;

    THREE.BufferGeometry g = THREE.BufferGeometry();
    THREE.CylinderGeometry m0 = THREE.CylinderGeometry(radius, radius, height, sRadius, 1, true);
    THREE.SphereGeometry m1 = THREE.SphereGeometry(radius, sRad, sHei, 0, o0, 0, o1);
    THREE.SphereGeometry m2 = THREE.SphereGeometry(radius, sRad, sHei, 0, o0, o1, o1);
    THREE.Matrix4 mtx0 = THREE.Matrix4().makeTranslation(0, 0,0);
    THREE.Matrix4 mtx1 = THREE.Matrix4().makeTranslation(0, height*0.5,0);
    THREE.Matrix4 mtx2 = THREE.Matrix4().makeTranslation(0, -height*0.5,0);

    g.merge(m0,0).applyMatrix4(mtx0);
    g.merge(m1,1).applyMatrix4(mtx1);
    g.merge(m2,2).applyMatrix4(mtx2);
    capsuleGeometry = g;
  }
  void clearMesh(){
  ...
  void populate() {
    ...
    if(type==1){// DEMO 1
      ...
      //var ground = OIMO.Body({size:[100, 40, 390], pos:[0,-20,0], world:world});
      //var ground2 = OIMO.Body({size:[400, 40, 400], pos:[0,-60,0], world:world});
      ...
      for(int i = 0; i < max; i++){
        ...
        if(j==1){
          meshs.add(THREE.Mesh( capsuleGeometry, matBox ));
        }
        else{ 
          meshs.add(THREE.Mesh( capsuleGeometry, matSphere ));
        }
      ...
      //b = OIMO.Body({type:'sphere', size:[80], pos:[0,1000,-600], move:true, world:world});
      //bodys[max] = b.body;
      bodys.add(world!.add(
